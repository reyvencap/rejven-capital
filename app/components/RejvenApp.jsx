...
function FundEditor({ fund, setFunds }) {
  ...
  function addH() {
    const s = +nsh, a = +nav;
    if (!ntk.trim() || !(s > 0) || !(a > 0)) return;
    const t = ntk.trim().toUpperCase();
    setFunds((prev) => prev.map((f) => (f.id !== fund.id ? f : { ...f, holdings: [...f.holdings, { tk: t, name: t, shares: s, avg: a, price: a, prev: a }] })));
    setNtk(""); setNsh(""); setNav("");
  }
  ...
  return (
    <div className="efund">
      ...
      <div className="hrow">
        <input className="num" style={{ width: 64 }} value={ntk} onChange={(e) => setNtk(e.target.value)} placeholder="TICK" aria-label="New ticker" />
        <input className="num" style={{ flex: 1 }} value={nsh} onChange={(e) => setNsh(e.target.value)} placeholder="shares" inputMode="decimal" aria-label="New shares" />
        <input className="num" style={{ flex: 1 }} value={nav} onChange={(e) => setNav(e.target.value)} placeholder="avg cost" inputMode="decimal" aria-label="New average cost" />
        <button className="iconbtn" title="Add holding" aria-label="Add holding" onClick={addH} style={{ borderColor: "var(--brand)", color: "var(--brand)" }}><Plus size={14} /></button>
      </div>
    </div>
  );
}
